...
class ArticleController {
    ...
    public function show($id) {
        ...
        $allEntries = $this->commentRatingModel->getAllEntriesByItem($id, 'article');
        $comments = [];
        foreach ($allEntries as $entry) {
            if ($entry['rating_value'] === null) { // Filter for pure comments (where rating_value is NULL)
                $comments[] = $entry;
            }
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Pastikan parent_comment_id adalah NULL jika tidak ada atau kosong
            $parentCommentId = null; // Default to null for top-level comments
            if (!empty($_POST['parent_comment_id'])) {
                // Gunakan FILTER_NULL_ON_FAILURE untuk memastikan null jika input tidak valid
                $parentCommentId = filter_var($_POST['parent_comment_id'], FILTER_VALIDATE_INT, FILTER_NULL_ON_FAILURE);
            }
