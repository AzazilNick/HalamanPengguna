...
class AuthController {
    ...
    public function login() {
        // Poin 7: Jika sudah login, redirect ke dashboard
        if (Session::has('user')) {
            redirect('/dashboard'); // Menggunakan helper redirect
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($usernameOrEmail) || empty($password)) {
                ...
                if ($user && password_verify($password, $user['password'])) {
                    ...
                    // Poin 4: Error login tidak disimpan di session, langsung ditampilkan
