...
require_once APP_ROOT . '/app/Models/Article.php'; // Tambahkan ini
...
class CommentRatingController {
    ...
    private $filmModel;
    private $seriesModel;
    private $articleModel; // Tambahkan ini
    ...
    public function __construct(PDO $pdo) {
        ...
        $this->articleModel = new Article($pdo); // Tambahkan inisialisasi ini
    ...
    public function detail($itemType, $itemId) {
        ...
        } elseif ($itemType === 'article') { // Tambahkan blok ini
            $item = $this->articleModel->findById($itemId); // Muat artikel
            if ($item) {
                // Mengambil semua komentar dan rating sekaligus
                $allEntries = $this->commentRatingModel->getAllEntriesByItem($itemId, 'article');
                // Artikel tidak memiliki "review" (rating khusus), jadi userReviewEntry tetap null atau tidak digunakan
                $isLiked = false; // Artikel tidak memiliki like terpisah dari komentar
            }
        } else {
        ...
        if (!$item) {
            redirect('/komentar_rating?message=' . urlencode('Film/Series/Artikel tidak ditemukan.') . '&type=error');
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($action === 'submit_comment_review') { // Handle submission of combined comment/review
                ...
                $parentCommentId = null; // Default to null for top-level comments
                if (isset($_POST['parent_comment_id'])) {
                    $rawParentId = $_POST['parent_comment_id'];
                    // Pastikan nilai adalah numerik dan positif sebelum menggunakannya sebagai ID
                    if (is_numeric($rawParentId) && (int)$rawParentId > 0) {
                        $parentCommentId = (int)$rawParentId;
                    }
                }
                ...
                if (empty($commentText) && ($ratingValue === false || $ratingValue < 1 || $ratingValue > 10)) {
                    ...
                    if ($ratingValue !== null && $parentCommentId === null && ($itemType === 'film' || $itemType === 'series')) { // Hanya film/series yang punya rating ulasan
                        ...
                        // Ini adalah komentar biasa (bisa jadi balasan) atau komentar untuk artikel
                ...
                // Artikel tidak memiliki toggle_like untuk item utama, hanya komentar
    ...
    public function deleteEntry($itemType, $entryId) { // Diganti dari deleteRating/deleteComment
        if (!Session::has('user')) {
        ...
        // Redirect kembali ke halaman detail item asalnya
        redirect('/komentar_rating/detail/' . $entry['item_type'] . '/' . $entry['item_id'] . '?message=' . urlencode($message) . '&type=' . urlencode($messageType));
    }
}