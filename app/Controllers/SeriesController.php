...
class SeriesController {
    ...
    public function validateFieldAjax() {
        ...
        $response = ['valid' => true, 'message' => '']; // Default: valid, tanpa pesan

        if (!Session::has('user') || Session::get('user')['is_admin'] != 1) {
            // Jika tidak login atau bukan admin, kirim respons validasi gagal dan redirect
        ...
        $seriesId = filter_var($_POST['seriesId'] ?? null, FILTER_VALIDATE_INT);

        switch ($fieldName) {
            case 'title':
                if (empty(trim($fieldValue))) { // Gunakan trim() untuk memastikan spasi kosong juga dianggap kosong
                    $response = ['valid' => false, 'message' => 'Judul series tidak boleh kosong.'];
                } else if (strlen(trim($fieldValue)) < 3) { // Contoh: minimal 3 karakter
                    $response = ['valid' => false, 'message' => 'Judul series terlalu pendek.'];
                }
                // Anda bisa menambahkan validasi unik di sini jika diperlukan,
                // dengan membandingkan $fieldValue dengan judul series lain (kecuali series yang sedang diedit)
                // contoh: if ($this->seriesModel->titleExists($fieldValue, $seriesId)) { ... }
                break;
            case 'release_year':
                $year = filter_var($fieldValue, FILTER_VALIDATE_INT);
                $currentYear = date('Y');
                if ($year === false || $year < 1888 || $year > $currentYear + 5) { // Contoh: tahun tidak boleh sebelum 1888 (awal era film) dan tidak lebih dari 5 tahun di masa depan
                    $response = ['valid' => false, 'message' => 'Tahun rilis tidak valid (contoh: 1888 - ' . ($currentYear + 5) . ').'];
                }
                break;
            case 'image_url':
                if (!empty($fieldValue) && !filter_var($fieldValue, FILTER_VALIDATE_URL)) {
                    $response = ['valid' => false, 'message' => 'Format URL gambar tidak valid.'];
                }
                break;
            default:
                $response = ['valid' => false, 'message' => 'Field validasi tidak dikenal.'];
                break;
        }
