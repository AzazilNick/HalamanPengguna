...
<main>
    <div class="article-detail-container">
        ...
        <article class="single-article">
            ...
            <p class="<?= $itemType ?>-meta">Rating Rata-rata: <strong><?= number_format($item['average_rating'], 1) ?>/10</strong> (dari <?= escape_html($item['total_new_ratings']) ?> ulasan)</p>
        ...
        <section class="comments-section">
            <h2>Ulasan Pengguna (<?= escape_html($item['total_new_ratings']) ?>)</h2>
            ...
            <div class="comment-form">
                ...
                <form action="<?= $basePath ?>/komentar_rating/detail/<?= escape_html($item_type) ?>/<?= escape_html($item['id']) ?>" method="POST">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" id="rating_value" name="rating_value" min="1" max="10" value="<?= escape_html($currentRating) ?>" required>
                    </div>
                    <div class="input-group">
                        <label for="review_text">Ulasan:</label>
                        <textarea name="comment_text" id="review_text" placeholder="Tulis ulasan Anda di sini..." rows="8" required><?= escape_html($currentReviewText) ?></textarea>
                    </div>
        ...
        <section class="comments-section" style="margin-top: 30px;">
            <h2>Komentar Umum (<?= escape_html($item['total_comments']) ?>)</h2>
            ...
            // Fix: Pass $itemType to renderComments function
            renderComments($comments, $basePath, $itemType, $item['id'], Session::get('user'), $pdo);
            ?>
        </section>
    </div>
</main>
