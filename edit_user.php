...
// apabila id sudah di set, maka akan menyimpan nilai id bertipe int
// apabila id belum di set maka akan menyimpan nilai 0
$user_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
$current_user = null;
// jika user_id bernilai 1
if ($user_id) {
    $stmt = $conn->prepare("SELECT * FROM user WHERE id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    // variable stmt diambil result nya (harus hanya 1 baris saja)
    // dan disimpan ke variable current_user
...
$message = '';
$error = '';
// jika request method post dan variable current_user sudah bernilai
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $current_user) {
    // menyimpan nilai dari variable username, email, nama_lengkap dengan spasi yang dihapus(jika ada)
    ...
    if (empty($username) || empty($email)) {
        ...
        // jika setelah berhasil di execute
        if ($stmt->execute()) {
            $message = "Akun berhasil diperbarui!";
            // Update data current_user untuk form
            // isi dari variable diupdate ke data current_user
