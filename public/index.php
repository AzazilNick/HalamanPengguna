        ...
        switch ($uriSegments[0]) {
            ...
            case 'comment': // Handle 'comment/delete/{id}' specifically for article comments
                ...
                if (!empty($uriSegments[0]) && $uriSegments[0] === 'delete' && !empty($uriSegments[1])) {
                    ...
                    $commentIdToDelete = array_shift($uriSegments); // Get the ID

                    // Sebelum memanggil deleteEntry, kita perlu tahu item_id dan item_type dari komentar tersebut.
                    // Ini memerlukan pencarian komentar di database terlebih dahulu.
                    // Ini tidak bisa dilakukan di sini langsung, karena controller belum di-instantiate.
                    // Solusi terbaik adalah membiarkan CommentRatingController menangani redirect.
                    // Kita hanya perlu memastikan bahwa deleteEntry dipanggil dengan benar.

                    // Parameter yang diteruskan ke deleteEntry adalah ($itemType, $entryId)
                    // Disini kita hanya punya $entryId.
                    // Karena itu, kita harus memodifikasi deleteEntry di CommentRatingController
                    // agar dia bisa mencari item_type dan item_id sendiri berdasarkan $entryId.

                    $params = [$commentIdToDelete]; // Hanya teruskan entryId
                } else {
