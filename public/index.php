    ...
    // Jika tidak ada maka
        ...
        switch ($uriSegments[0]) {
            case 'articles':
                ...
                break;
            case 'comment': // Keep this for existing ArticleController delete functionality
            ...
            case 'komentar_rating':
                $controllerName = 'KomentarRatingController';
                $controllerFilePath = APP_ROOT . '/app/Controllers/KomentarRatingController.php';
                array_shift($uriSegments);
                break;
            default:
                break;
...
if (method_exists($controller, $actionName)) {
    // Special routing for KomentarRatingController
    if ($controllerName === 'KomentarRatingController') {
        ...
        } elseif ($actionName === 'deleteRating' && count($params) >= 2) {
            $itemType = array_shift($params);
            $ratingId = array_shift($params);
            call_user_func_array([$controller, $actionName], [$itemType, $ratingId]);
        } elseif ($actionName === 'deleteComment' && count($params) >= 1) { // Only takes commentId
            $commentId = array_shift($params);
            call_user_func_array([$controller, $actionName], [$commentId]);
        } else {
