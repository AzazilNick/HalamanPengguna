        ...
        switch ($uriSegments[0]) {
            ...
            case 'comment': // Handle 'comment/delete/{id}' specifically for article comments
                ...
                if (!empty($uriSegments[0]) && $uriSegments[0] === 'delete' && !empty($uriSegments[1])) {
                    ...
                    array_shift($uriSegments); // Remove 'delete' segment
                    $commentIdToDelete = array_shift($uriSegments); // Get the ID
                    $params = ['article', $commentIdToDelete]; // Pass 'article' as itemType and the ID
                } else {
...
if (method_exists($controller, $actionName)) {
    ...
    if ($controllerName === 'CommentRatingController') {
        ...
        } elseif ($actionName === 'deleteEntry' && count($params) >= 2) { // Already handles deleteEntry
            $itemType = array_shift($params);
            $entryId = array_shift($params);
            call_user_func_array([$controller, $actionName], [$itemType, $entryId]);
        } else {
